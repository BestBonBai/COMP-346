...
public class Network extends Thread {
    ...
    private static Semaphore s_emptySending;
    private static Semaphore s_fullReceiving;
    private static Semaphore s_fullTransferIn;
    private static Semaphore s_emptyTransferOut;
     ...
     Network( )
      { 
         ...
         networkStatus = "active";
          ...
          /*TODO: initial semaphore*/
          s_emptySending = new Semaphore(maxNbPackets); //empty count
          s_fullTransferIn = new Semaphore(0); //full count
          mutexIn = new Semaphore(1);

          s_emptyTransferOut = new Semaphore(maxNbPackets); //empty count
          s_fullReceiving = new Semaphore(0); //full count
     ...
     */
        ...
        {
        	/* TODO: Using the method acquire() and release() of the class Semaphore,
        	    synchronize the operations of the network input buffers.
        	    The semaphores must be implemented in the methods send(), receive(), transfer() and transferOut() of the class Neywork*/
        	//block a thread if P1 less or equal to 0
        	try {
        	    s_emptySending.acquire();
        	    mutexIn.acquire();
            ...
            /*----------------------------------------------*/
                // V (signal())
                mutexIn.release();
                s_fullTransferIn.release();
            }
            catch (InterruptedException e){
                System.out.println("Interruption on send() method");
            }
     ...
     */
         public static boolean receive(Transactions outPacket){
                ...
                * The semaphores must be implemented in the methods send(), receive(), transfer() and transferOut() of the class Neywork*/
                //P() (wait())
                try{
                    s_fullReceiving.acquire();
                    mutexOut.acquire();
                }catch(InterruptedException e){
                    System.out.println("Interruption on receive() method");
                    e.printStackTrace();
                }
     ...
     */
         public static boolean transferOut(Transactions outPacket){
            ...
            /*TODO: Using the method acquire() and release() of the class Semaphore, synchronize the operations of the network input buffers.*/
        	// block a thread if P3 (wait()) less or equal to 0
            try{
                s_emptyTransferOut.acquire();
                mutexOut.acquire();
            }
            catch(InterruptedException e){
                System.out.println("Interruption on transferOut()");
                e.printStackTrace();
            }
    ...
    {	
        ...
        /*TODO: PA1_Implement the code for the run method */

    	while (true)
    	{
    		/*................................................................................................................................................................*/

            // The terminal condition to break the infinite loop of Network thread
            if (getClientConnectionStatus().equals("disconnected") && getServerConnectionStatus().equals("disconnected")){
                ...
                // Network thread yields its current use of a processor continuously
                Thread.yield();
            }
    	}
