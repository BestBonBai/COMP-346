...
public class Server extends Thread{
     ...
     */  
     public void initializeAccounts()
     {
        ...
        try
        {
         inputStream = new Scanner(new FileInputStream("comp346pa1w2020/account.txt"));
        }
...
//               Thread.yield();
             //if the network input buffer is empty, the thread (server) yield the CPU, waiting the client send transactions to the input buffer;
             if((objNetwork.getInBufferStatus().equals("empty"))) {
                 Thread.yield();
             } //tried the above while method, but it fell into an infinite loop.
             /*--------------------------------------------------*/
        	 ...
        	 if (!objNetwork.getInBufferStatus().equals("empty"))
        	 {
                 ...
                 // if the network output buffer is full, the thread (server) yield the CPU. waiting the client receive transactions from the output buffer;
                 /*------------------------------------------------------------------------------*/
    ...
    public void run()
    {
        ...
        if(processTransactions(trans)) {
            ...
            //set the status of the Sever thread to disconnected
