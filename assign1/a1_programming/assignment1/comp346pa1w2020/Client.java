...
public class Client extends Thread{
     ...
     */
     public void readTransactions()
     {
        ...
        try
        {
        	inputStream = new Scanner(new FileInputStream("comp346pa1w2020/transaction.txt"));
        }
     ...
     */
     public void sendTransactions()
     {
         ...
         {
             /*TODO: Implement the code for busy-wait until the network input buffer is available */
            ...
            while( objNetwork.getInBufferStatus().equals("full")) {
                Thread.yield();
            } //if the network input buffer is full, the thread (client) yield the cpu, waiting the server process transactions from the input buffer;
            /*-----------------------------------------------------------*/
     ...
     */
     public void receiveTransactions(Transactions transact)
     {
         ...
         while (i < getNumberOfTransactions())
         {
             /*TODO: Implement the code for busy-wait until the network output buffer is available */
             ...
             while( objNetwork.getOutBufferStatus().equals("empty")){
                 Thread.yield();
             }//if the network output buffer is empty, the thread yield the cpu;
    ...
    {   
        ...
        //for the client thread sending
        if (clientOperation.equals("sending")){
            //get sendClientStartTime and sendClientEndTime to record the running times of the client thread sending
            //using the method System.currentTimeMillis()
            sendClientStartTime = System.currentTimeMillis();
            //System.out.println("\n DEBUG : Client.run() - starting client sending thread connected");

            //send transactions to input buffer
            sendTransactions();
        ...
        //for receive client thread
        else if (clientOperation.equals("receiving")){
            receiveClientStartTime = System.currentTimeMillis();
            //System.out.println("\n DEBUT : Client.run() - starting client receiving thread connected");

            //receive transactions to input buffer
            receiveTransactions(transact);

            receiveClientEndTime = System.currentTimeMillis();
            System.out.println("\n Terminating client receiving thread - " + " Running time " + (receiveClientEndTime - receiveClientStartTime) + " milliseconds");

            //set the status of the receive Client thread to disconnected
            objNetwork.setClientConnectionStatus("disconnected");
        }
